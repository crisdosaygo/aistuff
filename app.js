...
const APP_DEFINITIONS = {
    ...
    internetBrowser: {
        title: "Internet Browser", // Simplified, BrowserApp handles its own title nuances
        ...
        initApp: (windowEl, windowInstanceId, webviewId, appDefinition) => {
            return new BrowserApp(windowEl, windowInstanceId, webviewId, false, appDefinition); // Pass false for netscapeFlag_unused
        }
    },
    internetExplorer: {
    ...
    netscapeNavigator: {
        ...
        generateContent: (windowInstanceId, webviewId) => BrowserApp.generateInitialHTML(webviewId),
        initApp: (windowEl, windowInstanceId, webviewId, appDefinition) => {
            // For Netscape, you might pass 'true' to BrowserApp if it uses that flag for styling/behavior
            return new BrowserApp(windowEl, windowInstanceId, webviewId, true, appDefinition);
        }
    },
};
window.APP_DEFINITIONS = APP_DEFINITIONS; // Expose globally for other modules like desktop-icons.js
    ...
    const promises = imageUrls.map(src => {
        return new Promise((resolve) => {
            ...
            img.onload = img.onerror = () => {
                if (img.onerror && !img.onload) console.warn(`Failed to preload image: ${src}`);
                loadedCount++;
                if (typeof onProgress === 'function') onProgress((loadedCount / totalAssets) * 100);
                resolve();
            };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function createWindow(appId, dataForApp) {
        ...
        windowEl.querySelector('.window-title').textContent = appDef.title; // Simplified, BrowserApp might override
    ...
    function makeResizable(element) {
        ...
        handles.forEach(handle => {
            handle.addEventListener('pointerdown', (e) => {
                ...
                element.setPointerCapture(e.pointerId); // Capture pointer on the handle
            });
        });
    
        // Changed to element-specific listeners for move and up
        element.addEventListener('pointermove', (e) => {
            if (!isResizing || !currentHandle) return;
            // e.preventDefault(); // Keep commented if it causes jank
        ...
        element.addEventListener('pointercancel', (e) => { // Handle unexpected pointer release
            if (isResizing) {
                isResizing = false; currentHandle = null;
                document.body.style.cursor = 'default';
                document.body.classList.remove('no-select');
    ...
    if (window.Win9xDesktopUtils && typeof window.Win9xDesktopUtils.forceRelayoutAllIcons === 'function') {
        console.log("Ensuring desktop icons are laid out...");
        window.Win9xDesktopUtils.forceRelayoutAllIcons();
    } else {
        console.warn("Win9xDesktopUtils.forceRelayoutAllIcons not found during initial setup.");
    }
